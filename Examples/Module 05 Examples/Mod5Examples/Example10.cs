...
namespace Mod5Example10 {
    ...
    class Zoo {
        public Animal[] Animals { get; } = new Animal[ 100 ];
        public int AnimalListCount { get; set; } = 0;
                        ...
                        if (animal.Name != null) {
                            ...
                            for (; i < AnimalListCount; i++) {
                                if (Animals[ i ].Name.Equals(animal.Name, StringComparison.OrdinalIgnoreCase)) {
                                    ...
                                    Animals[ i ].Count = animal.Count;
                                    Animals[ i ].Value = animal.Value;
                                    break;
                                }
                            }
                            if (i == AnimalListCount) {
                                Animals[ AnimalListCount ] = new Animal { Name = animal.Name, Count = animal.Count, Value = animal.Value };
                                AnimalListCount++;
                            }
                        }
                        break;
        ...
        private (string? Name, int Count, decimal Value) AddAnimalToZoo() {
            ...
            if (string.IsNullOrEmpty(animalName)) {
                ...
                return (Name: animalName, Count: animalCount, Value: animalValue);
            }
        }
